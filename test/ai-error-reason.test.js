...
import { categorizeAiError } from '../src/lib/utils/ai-error-reason.js';

describe('categorizeAiError', () => {
  test('maps each openai SDK error subclass to its constructor name', () => {
    expect(
      categorizeAiError(
        new AuthenticationError(401, { error: { message: 'bad key' } }, 'Unauthorized', new Headers())
      )
    ).toBe('AuthenticationError');
    expect(
      categorizeAiError(
        new RateLimitError(429, { error: { message: 'slow down' } }, 'Too many requests', new Headers())
      )
    ).toBe('RateLimitError');
    expect(
      categorizeAiError(new APIConnectionError({ message: 'Connection error.' }))
    ...
    expect(
      categorizeAiError(
        new InternalServerError(500, { error: { message: 'oops' } }, 'Internal server error', new Headers())
      )
    ).toBe('InternalServerError');
  });

  test('collapses "empty response" failures to the same code regardless of message', () => {
    expect(categorizeAiError(new Error('Empty response from AI provider'))).toBe(
      'empty_response'
    );
  });
  ...
  test('collapses "invalid JSON" failures to the same code despite varying interpolated detail', () => {
    const a = categorizeAiError(
      new Error('AI response is not valid JSON: Unexpected token < in JSON at position 0')
    );
  ...
  test('collapses "missing score" failures to the same code despite varying interpolated reply text', () => {
    const a = categorizeAiError(
      new Error('AI response missing numeric "score" field: {"reasoning":"looks fine"}')
    );
    const b = categorizeAiError(
  ...
  test('falls back to "unknown" for an unrecognized error', () => {
    expect(categorizeAiError(new TypeError('something else broke'))).toBe('unknown');
  });
});
