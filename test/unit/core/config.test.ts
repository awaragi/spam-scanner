...
import { configGroups } from '../../../src/lib/core/config.ts';
import { renderEnvFile } from '../../../src/lib/utils/env-file.util.ts';
...
describe('config', () => {
  ...
  test('SPAM_PROCESSING_MODE defaults to "folder" when unset', async () => {
    ...
    const { config } = await import('../../../src/lib/core/config.ts');
  ...
  test('throws at load time when AI_ENABLED=true and AI_MODEL is unset', async () => {
    ...
    await expect(import('../../../src/lib/core/config.ts')).rejects.toThrow(
      /AI_MODEL/
    );
  });
