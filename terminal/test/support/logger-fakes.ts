 ...
 * matches `core/logger.ts`'s `Logger`/`ComponentLogger`/`RootLogger` shape
 * closely enough for test doubles (all methods are `vi.fn()` spies, so
 * assertions like `expect(logger.warn).toHaveBeenCalled()` work directly).
 */
...
/**
 * Builds a no-op logger for `vi.mock('.../core/logger.ts', ...)` factories.
 ...
 * lifetime.
 */
export function createNoOpLogger(overrides: Partial<NoOpLogger> = {}): NoOpLogger {
 ...
 * `rootLogger` from `core/logger.ts` is mocked as everywhere in this
 * codebase's tests.
 */
