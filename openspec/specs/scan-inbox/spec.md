# scan-inbox Specification

## Purpose

Governs how the scan workflow selects which inbox messages to process on each pass and reports back how many it processed, so a caller can drive repeated passes (e.g. an IDLE drain loop) until the inbox is caught up.
