...
Governs how environment-variable-sourced configuration is validated: what a declarative
schema checks, when each check runs (at module load vs. an explicit operator-facing
startup check), and what a caller sees when validation fails - so misconfiguration is
caught early with a complete, readable list of problems instead of one cryptic failure at
a time.
